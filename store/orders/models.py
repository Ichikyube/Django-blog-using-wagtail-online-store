...
class OrderManager(models.Manager):
    ...
    def new_or_get(self, billing_profile, cart_obj):        
        created = False
        qs = self.get_queryset().filter(billing_profile=billing_profile, cart=cart_obj, active=True, status='created')#.exclude(status = "paid")#Order.objects.filter(billing_profile=billing_profile, cart=cart_obj, active=True)
        if qs.count() == 1:
            obj = qs.first()
        else:
            obj= self.model.objects.create(billing_profile=billing_profile, cart=cart_obj)#Order.objects.create(billing_profile=billing_profile, cart=cart_obj)
            created = True
        return obj, created
...
class Order(models.Model):
    ...
    shipping_total   = models.DecimalField(default=5.22, max_digits=100, decimal_places=2)
    ...
    def update_total(self):
        ...
        new_total = format(Decimal(cart_total) + Decimal(shipping_total), '.2f')
    ...
    def check_done(self):
        ...
        if billing_address and shipping_done and billing_profile and total > 0:
            return True
        return False
