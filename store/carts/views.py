...
def cart_home(request):   
    cart_obj, new_obj = Cart.objects.new_or_get(request)
    #request.session['total'] = format(cart_obj.total, '.2f')
...
def cart_update(request):
    ...
    if product_id is not None:
        try:
            ...
            print("I'm sorry, we cannot find the specified product")
        ...
        if product_obj in cart_obj.products.all(): 
            ...
            cart_obj.products.add(product_obj) # car_obj.products.add(product_id)
            added = True
    ...
    return redirect('cart:home')#return redirect(product_id.get_absolute_url())
